...
0000000000400ee0 <phase_1>: #知道edi存的是 用户输入   esi存的是 密文，那么结果显而易见
  ...
  400eee:	85 c0                	test   %eax,%eax   #eax内容如果是0那么结束
...
0000000000400f43 <phase_3>:
  ...
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt> #sscanf的返回值 是 获取成功的输入的个数。
  400f60:	83 f8 01             	cmp    $0x1,%eax #成功 输入个数 必须>1，否则会爆炸
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp) #如果 mem(0x8(%rsp))＞7，也就是用户第一个输入的值>7, 那么爆炸
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax #把用户输入的第一个值 放到 eax中
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8) #间接跳转，以 mem(0x402470(,%rax,8))为目标

#如果第一个值为1，那么跳转到 400fb9 ；如果是2。按照1来走，就能找到结果了，所以后面的情况没看...
  ...
  400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax #如果第2个数 和 0x137(311)相等，那么就不会爆炸
...
# <read_six_numbers>的作用是 拿到用户的6个int，并放入 (rsp) (rsp+4) (rsp+8) (rsp+12) (rsp+16) (rsp+20)
...
00000000004010f4 <phase_6>:
  ...
  4010fa:	55                   	push   %rbp
  4010fb:	53                   	push   %rbx
  4010fc:	48 83 ec 50          	sub    $0x50,%rsp #80 rsp上面预留80字节的空间
  401100:	49 89 e5             	mov    %rsp,%r13
  401103:	48 89 e6             	mov    %rsp,%rsi
  401106:	e8 51 03 00 00       	callq  40145c <read_six_numbers> #输入的是6个数字
  ...
  401158:	4c 89 f0             	mov    %r14,%rax #!!!joe: 可能出错的指令
  40115b:	b9 07 00 00 00       	mov    $0x7,%ecx

  401160:	89 ca                	mov    %ecx,%edx
  401162:	2b 10                	sub    (%rax),%edx #这个地方 (%rax)应该是取4个字节？
  401164:	89 10                	mov    %edx,(%rax)
  401166:	48 83 c0 04          	add    $0x4,%rax

  40116a:	48 39 f0             	cmp    %rsi,%rax
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>
  40116f:	be 00 00 00 00       	mov    $0x0,%esi #joe: 可能出错的地方
  401174:	eb 21                	jmp    401197 <phase_6+0xa3>
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx #每次的值都是一样的，相当于给rdx直接赋值为6304480
  ...
  4011d2:	48 c7 42 08 00 00 00 	movq   $0x0,0x8(%rdx) #这条指令 有什么用吗？？？
  4011d9:	00 
...
0000000000401338 <strings_not_equal>:  #只有最后%eax返回是0，才是相等的
  401338:	41 54                	push   %r12   #callee-saved
  40133a:	55                   	push   %rbp   #callee-saved
  40133b:	53                   	push   %rbx   #callee-saveds
  40133c:	48 89 fb             	mov    %rdi,%rbx    #用户输入的文字的位置属性 放在%rdi中。保存一下传过来的rdi
  40133f:	48 89 f5             	mov    %rsi,%rbp    #密文的位置属性 放在%rsi中的
  401342:	e8 d4 ff ff ff       	callq  40131b <string_length>
  401347:	41 89 c4             	mov    %eax,%r12d   #eax得到的是 输入的字符的长度
  40134a:	48 89 ef             	mov    %rbp,%rdi
  40134d:	e8 c9 ff ff ff       	callq  40131b <string_length>
  401352:	ba 01 00 00 00       	mov    $0x1,%edx
  401357:	41 39 c4             	cmp    %eax,%r12d   #eax得到密文的 长度为54个字节
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63>
  40135c:	0f b6 03             	movzbl (%rbx),%eax
  40135f:	84 c0                	test   %al,%al #测试%al是否为0  如果是0，就是读取到 用户输入的结束了，那么eax赋为0，测试成功
  401361:	74 25                	je     401388 <strings_not_equal+0x50>
  401363:	3a 45 00             	cmp    0x0(%rbp),%al #检查 密文 和 用户输入 的某一个字节是否相同。rbp是密文，al是用户的输入
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a> #如果相同，检查下一位
  401368:	eb 25                	jmp    40138f <strings_not_equal+0x57>
  40136a:	3a 45 00             	cmp    0x0(%rbp),%al #开始循环 对比 密文 和 用户输入的每一个字节
  40136d:	0f 1f 00             	nopl   (%rax) #好像是一个空操作
  401370:	75 24                	jne    401396 <strings_not_equal+0x5e> #检查到 密文 和 用户输入不相同
  401372:	48 83 c3 01          	add    $0x1,%rbx #如果相同，检查下一字节。用户输入的位置+1
  401376:	48 83 c5 01          	add    $0x1,%rbp #如果相同，检查下一字节。密文的位置+1
  40137a:	0f b6 03             	movzbl (%rbx),%eax
  40137d:	84 c0                	test   %al,%al
  40137f:	75 e9                	jne    40136a <strings_not_equal+0x32> #如果是0，就是读取到 用户输入的结束了，那么eax赋为0，测试成功
  ...
  401396:	ba 01 00 00 00       	mov    $0x1,%edx #把%eax设为1
